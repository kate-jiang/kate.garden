...
import { config, dayConfig, nightConfig, linkData, playlist } from "@/config";
...
function showOverlay(contentType: "about" | "music"): void {
  ...
  if (contentType === "about") {
    ...
    setTimeout(() => {
      const activeItem = document.querySelector(".playlist-items .playlist-item.active");
      if (activeItem) {
...
canvas.addEventListener("pointerdown", e => {
  ...
  if (!hasAutoPlayed && player.getPreference() === "true") {
    hasAutoPlayed = true;
    player.playAudio();
  }
...
const player = createMusicPlayer({
  playlist,
  onNowPlayingClick: () => showMusicPanel(),
